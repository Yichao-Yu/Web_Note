...
chrome.runtime.onInstalled.addListener(function (object) {
	...
	for (var i = 0; i< contexts.length; i++) {
		...
		var id = chrome.contextMenus.create({"title": "Add to Note", "contexts":[context], "id": context + "_context"});
		chrome.contextMenus.create({"title": "Add to Recent notes...", "contexts":[context], "parentId": context + "_context", "id": context + "_recent_notes"});
		chrome.contextMenus.create({"title": "Open a new Note", "contexts":[context], "parentId": context + "_context", "id": context + "_new_note"});
		// mock up the exsting recent notes
		...
		// separator between recent notes and open existing note
		chrome.contextMenus.create({"type": "separator", "contexts":[context], "parentId": context + "_recent_notes", "id": context + "_recent_existing_separator"});
